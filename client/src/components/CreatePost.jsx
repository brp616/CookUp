...
export default function CreatePost({
  ...
  const handleImageUpload = async (e) => {
    ...
    if (invalidFile) {
      alert(
        "Only image files (JPG, PNG, WEBP, GIF) are allowed! Please remove the PDF or other files.",
      );
  ...
  const handleSubmit = async (e) => {
    ...
    const newCook = {
      ...
      userAvatar:
        user.profilePic ||
        "https://api.dicebear.com/7.x/avataaars/svg?seed=" + user.username, // Added: Matches schema field
      recipeName,
      description,
